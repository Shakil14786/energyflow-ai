import { Ship, Fuel, TriangleAlert, Clock3, Sparkles, CircleCheck, CircleDashed } from 'lucide-react'
        ...
        <div ref={ref} data-reveal-group className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="glass rounded-2xl p-6 lg:col-span-2">
            ...
            <ul className="space-y-3">
              {SHIPMENTS.map((s) => (
                <li key={s.id} className="flex items-center justify-between rounded-xl bg-white/5 border border-line px-4 py-3">
                  <div>
          ...
          <div className="glass rounded-2xl p-6 flex flex-col">
            <h3 className="font-medium flex items-center gap-2 mb-5">
              <TriangleAlert className="w-4 h-4 text-accent" aria-hidden="true" /> Risk meter
            </h3>
              ...
              {ROUTES.map((r) => (
                <li
                  ...
                  className={`flex items-center justify-between rounded-xl px-4 py-3 border ${
                    r.disabled ? 'border-line opacity-40' : 'border-primary/25 bg-primary/5'
                  }`}
                >
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <div className="flex items-end gap-1.5 h-24">
              {FUEL.map((h, i) => (
                <div key={i} className="flex-1 rounded-t bg-gradient-to-t from-primary/60 to-accent/90" style={{ height: `${h}%` }} />
              ))}
            </div>
          ...
          <div className="glass rounded-2xl p-6 border-primary/30">
            <h3 className="font-medium flex items-center gap-2 mb-4">
              <Sparkles className="w-4 h-4 text-accent" aria-hidden="true" /> AI recommendation
            </h3>
            ...
            <button className="w-full text-sm font-medium rounded-lg bg-primary hover:bg-primary-dim transition-colors py-2.5">
